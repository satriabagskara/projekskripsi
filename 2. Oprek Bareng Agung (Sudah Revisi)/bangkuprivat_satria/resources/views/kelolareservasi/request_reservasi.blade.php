              ...
              <table id="dataTable" class="table table-bordered table-hover text-center" width="100%" cellspacing="0">
                ...
                <tbody>
                  ...
                  @foreach($data_reservasi as $key => $val)
                    <tr>
                      <td>{{$no++}}.<a href="{{ url('/reservasi/detail/'.$val->id) }}" type="button" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                      </td>
                      ...
                      <td>
                        @if(auth()->user()->level_id == 3)
                          @if($val->status_id == 3)
                            <a href="{{ url('/request_reservasi/approve/'.$val->id) }}" type="button" class="btn btn-sm btn-success" title="Terima Reservasi"><i class="fas fa-check"></i></a>
                            <a type="button" data-toggle="modal" data-target="#Modal_Penolakan" class="btn btn-sm btn-danger" title="Tolak Reservasi" onclick="menolak_reservasi(this);"><i class="fa fa-times"></i></a>
                          ...
                          @elseif($val->status_id == 8)
                            <a href="{{ url('/request_reservasi/approve/'.$val->id) }}" type="button" class="btn btn-sm btn-success" title="Terima Pembayaran"><i class="fas fa-check"></i></a>
                            <a type="button" data-toggle="modal" data-target="#Modal_Penolakan" class="btn btn-sm btn-danger" title="Tolak Pembayaran" onclick="menolak_reservasi(this);"><i class="fa fa-times"></i></a>
                            ...
                            <a type="button" data-toggle="modal" data-target="#Modal_Cek_pembayaran" class="btn btn-sm btn-info text-light" title="Cek Bukti Pembayaran" onclick="cek_pembayaran(this);"><i class="fa fa-search"></i> Cek</a>
                            @endif
                          @endif
      ...
      <div class="modal-header">
        ...
        <div class="modal-body">
          ...
          <div class="form-group row">
              ...
              <div class="col-md-6">
                  <textarea id="alasan_tolak" name="alasan_tolak" class="form-control" placeholder="Alasan Di Tolak" required></textarea>
              </div>
          </div>
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn btn-success" onclick="return confirm('Apakah Anda Yakin Menolak Reservasi?')"><i class="fa fa-check"></i> Tolak</button>
...
<script type="text/javascript">
  function menolak_reservasi(obj)
  {
       ...
       $('#Title_Penolakan').append('<h3 class="modal-title text-primary" id="Title_Penolakan">Menolak Reservasi, '+nama_user+'</h3>');
  ...
  function cek_pembayaran(obj)
  {
       ...
       $('#Title_Pembayaran').append('<h3 class="modal-title text-primary" id="Title_Pembayaran">Bukti Pembayaran, '+nama_user+'</h3>');
