...
<div class="content-header">
  <div class="container-fluid">
      <div class="row mb-2">
          <div class="col-sm-6">
              <h1 class="text-primary" style="font-size:40px;">CARI MENTOR RESERVASI</h1>
          </div>
          <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item"><a href="{{url('/home')}}">Home</a></li>
                  <li class="breadcrumb-item active">Cari Mentor Reservasi</li>
              </ol>
          </div>
      </div>
      <!-- Alert Status -->
  ...
  <div id="div_search" class="row ml-3 col-md-11">
    <div class="form-group col-md-5">
      <input id="subject" class="form-control" type="text" placeholder="Cari Berdasarkan Subject Nama atau Keahlian Mentor">
    </div>
    <div class="form-group col-md-5">
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body mt-3">
              <h2 id="data_tidak_tersedia" class="text-center text-danger" style="display:none;">Data Tidak Tersedia.</h2>
              @if($data == 0)
              ...
              @else
                <div id="data_mentor" class="row responsive" style="margin-bottom:auto;">
                    @foreach($data as $key => $value)
                      <a class="card ml-3 mr-2" style="width:30%;" href="{{url('/pengajuan/'.$value['id'])}}">
                          <div class="card">
                            @if($value["fhoto"] != null)
                              <?php $path = $value['path_fhoto'];
                                    $foto = $value['fhoto']; ?>
                              <img src="{{url('/Foto_Profile/'.$path.'/'.$foto)}}" alt="" style="width:100%; max-height:200px;">
                            @else
                              <img src="{{url('/TemplateHome/dist/img/user.jpg')}}" alt="" style="width:100%; max-height:200px;">
                            @endif
                            <p style="background-color:black;color:white;margin-bottom:-1%;">{{$value["nama"]}}</p>
                            <p style="background-color:black;color:white;margin-bottom:-1%;">{{$value["kota"]}}</p>
                            <p style="background-color:black;color:white;margin-bottom:-1%;">
                              <i class="bg-success mr-5"> Rp. {{ number_format($value["harga_perjam"],0,",",".") }}/Jam </i>
                              @if($value["total_bintang"] == null)
                                <b class="ml-5">0 <i class="fas fa-star" style="color:grey;"></i> ({{$value["jml_ulasan"]}} Ulasan)</b>
                              @else
                                <b class="ml-5">{{$value["total_bintang"]}} <i class="fas fa-star" style="color:orange;"></i> ({{$value["jml_ulasan"]}} Ulasan)</b>
                              @endif
                            </p>
                          </div>
                          <p style="text-align:justify">{{substr($value["detail_skill"], 0, 100)}}...</p>
                          <div class="btn-group" style="width:100%;margin-top:-2%;">
                              <p>
                                @foreach($value["skill"] as $val)
                                <button class="text-dark " type="button" disabled>{{$val->skill}}</button>
                                @endforeach
                              </p>
                          </div>
                      </a>
                  ...
                  if(tot_data != 0){
                     for(a=0; a<tot_data; a++)
                     {
                          ...
                          if(total_bintang == null){
                              total_bintang = "<b class='ml-5'>"+0+"<i class='fas fa-star' style='color:grey;'></i></b> (";
                          }else{
                              total_bintang = "<b class='ml-5'>"+total_bintang+"<i class='fas fa-star' style='color:orange;'></i></b> (";
                          }
                          if(path_fhoto != null){
                          ...
                          value = value + "<a class='card ml-3 mr-2' style='width:30%;' href='/pengajuan/"+id+"'>"
                                          +"<div class='card'>"
                                            +"<img src='"+url_fhoto+"' alt='' style='width:100%; max-height:200px;'>"
                                            +"<p style='background-color:black;color:white;margin-bottom:-1%;'>"+nama+"</p>"
                                            +"<p style='background-color:black;color:white;margin-bottom:-1%;'>"+kota+"</p>"
                                            +"<p style='background-color:black;color:white;margin-bottom:-1%;'>"
