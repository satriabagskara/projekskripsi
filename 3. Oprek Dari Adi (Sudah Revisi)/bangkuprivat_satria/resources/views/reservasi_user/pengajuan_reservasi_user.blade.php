    ...
    <div class="card" style="max-width:98%;">
      ...
      <div class="card-body">
        <div class="row">
          <div class="card col-sm-4 mb-3">
            <div class="media">
              <input id="id_mentor" type="hidden" name="id_mentor" value="{{$detail_mentor->id_mentor}}">
              ...
              @endif
                ...
                <div class="button-group ml-3">
                  ...
                  <a href="{{url('/cari_mentor')}}" type="button" class="btn btn-sm btn-danger"><i class="fa fa-times"></i> Cancel</a>
                  <a href="{{url('/pengajuan/reservasi/'.$detail_mentor->id_detail_mentor)}}" type="button" class="btn btn-sm btn-success"><i class="fa fa-check"></i> Mulai Pengajuan</a>
                  @else
                  <a href="{{url('/cari_mentor')}}" type="button" class="btn btn-sm btn-warning"><i class="fa fa-reply"></i> Back</a>
                  @endif
                </div>
            	</div>
            </div>
      ...
      <div class="card-body">
        <div class="row">
          ...
          <input id="jumlah_total_ulasan" type="hidden" name="jumlah_total_ulasan" value="{{count($ulasan_mentor)}}">
              ...
              @foreach($ulasan_mentor as $key => $val)
                <div class="row" style="margin-bottom:auto;">
                  <div class="col-sm-1 text-center">
                    @if($val->fhoto != null)
                      <a><img src="{{url('/Foto_Profile/'.$val->path_fhoto,$val->fhoto)}}" class="img-circle mt-2" style="max-height:70%;max-width:70%;"></a>
                    @else
                      <a><img src="{{url('/TemplateHome/dist/img/user.jpg')}}" class="img-circle mt-2" style="width:70%;"></a>
                    @endif
                  </div>
                  <div class="col-sm-9">
          ...
          <button id="btn_ulasan_banyak" type="button" class="btn btn-secondary text-center col-sm-12" name="button" onclick="tampil_ulasan_banyak();">Tampilkan Lebih Banyak</button>
        </div>
      </div>
      <!-- End Review -->
    </div>
                ...
                if(tot_data != 0){
                   for(a=0; a<tot_data; a++)
                   {
                        ...
                        for( a=0; bintang>a; bintang--){
                          hasil_bintang = hasil_bintang+"<i class='fas fa-star' style='color:orange;'></i> ";
                        }
                        if(bintang < 5){
                          var sisa_bintang = 5-data_ulasan[a].bintang;
                          for(b=0; sisa_bintang>b; sisa_bintang--){
                            hasil_bintang = hasil_bintang+"<i class='fas fa-star' style='color:grey;'></i> ";
                          }
                        }
                        value = value+"<div class='row' style='margin-bottom:auto;'>"
                                      +"<div class='col-sm-1 text-center'>"
                                          +"<a><img src='"+url_fhoto+"' class='img-circle mt-2' style='max-height:70%;max-width:70%;'></a>"
                                      +"</div>"
                                      +"<div class='col-sm-9'>"
