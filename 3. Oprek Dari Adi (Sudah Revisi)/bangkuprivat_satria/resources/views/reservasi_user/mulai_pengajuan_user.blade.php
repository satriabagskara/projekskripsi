            ...
            <form class="form-horizontal" action="{{url('/user/pengajuan/reservasi')}}" method="post">
              ...
              <div class="form-group row">
                  ...
                  <div class="col-md-12" style="margin-bottom:1%;">
                    ...
                    @foreach($kebutuhan as $value)
                      <input type="radio" class="kebutuhan" onclick="radio({{$value->id}});" name="kebutuhan" value="{{$value->id}}"> {{$value->kebutuhan}} </br>
                    @endforeach
                      <input type="radio" class="kebutuhan" onclick="radio(99);" name="kebutuhan" value="99"> Lainnya</br>
                      <input id="kebutuhan_lainnya" type="text" class="form-control" style="display:none;" name="kebutuhan_lainnya" placeholder="Silahkan Masukkan Yang Anda Butuhankan">
                  </div>
                  <div class="col-md-12" style="margin-bottom:3%;">
                    <label for="spek_kebutuhan">Ceritakan Spesifikasi Kebutuhan Anda</label>
                    <textarea id="spek_kebutuhan" class="col-md-12 form-control" name="spek_kebutuhan" maxlength="200" style="height:100%;" placeholder="Misal : Saya bukan developer tapi saya ingin belajar ngoding, saya butuh mentor yang bisa mengajari saya dari nol."
                    required></textarea>
                  </div>
                  <div class="col-md-12 mt-3 row" style="margin-bottom:1%;">
                    <div class="col-md-3">
                      ...
                      ?>
                      <input type="date" class="form-control" min="{{$date_now}}" name="tgl_class" required>
                    </div>
                    <div class="col-md-3">
                      <label for="">Pilih Jam</label>
                      <input type="time" class="form-control" name="jam_class" min="08:00" max="18:00" value="00:00" required>
                    </div>
                  </div>
                  <!-- <div class="col-md-12" style="margin-bottom:1%;">
                  ...
                  <div class="col-md-12" style="margin-bottom:1%;">
                    ...
                    @foreach($tipe_pengajaran as $val)
                      <input type="radio" class="ml-3" name="tipe_pengajaran" value="{{$val->tipe_pengajaran_id}}"> {{$val->tipe_pengajaran->tipe}}
                    @endforeach
                  </div>
                  <div class="col-md-12">
                    <label for="harga_jam">Jumlah Jam Yang Anda Perlukan</label>
                    <input id="jumlah_jam" class="col-md-3 form-control" type="text" name="jumlah_jam" placeholder="Jumlah Jam Yang Anda Perlukan" required>
                  </div>
                </div>
                <hr>
                <div class="form-group row">
                  <div class="col-md-12">
                    <p class="text-danger"><i>*Jumlah Jam Dikalikan dengan Jumlah Hari dan Dikalikan dengan Harga Reservasi /Jam.</i></p>
              ...
              <div class="form-row" style="margin-left:20%;">
                  <div class="form-group mt-4 mb-5 col-md-8">
                    <a type="submit" href="{{url('/cari_mentor')}}" class="btn btn-warning text-dark"><i class="fa fa-times"></i> Cancel</a>
...
console.log("Ready !");
  ...
  jumlah_jam.addEventListener("keyup", function(e) {
    $(this).val($(this).val().replace(/[^\d].+/, ""));
      ...
      if(jumlah > 8){
        alert("Maximal Hanya 8 jam Perhari.");
        $('#jumlah_jam').val("");
        $('#sub_total_view').empty();
        $('#sub_total_view').append('<h3 id="sub_total_view"><b>Sub Total : Rp. 0 </b></h3>');
        $('#sub_total').empty();
        ...
        var sub_total = jumlah*get_harga;
