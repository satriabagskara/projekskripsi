...
<div class="content-header">
  <div class="container-fluid">
      <div class="row mb-2">
          <div class="col-sm-8">
              <h1 class="text-primary" style="font-size:35px;">Pengajuan Perubahan Data Mentor</h1>
          </div>
          <div class="col-sm-4">
              <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item"><a href="{{url('/home')}}">Home</a></li>
                  <li class="breadcrumb-item active">Pengajuan Perubahan Data Mentor</li>
              </ol>
          </div>
      </div>
      <!-- Alert Status -->
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body mt-3">
            <form class="form-horizontal" action="{{url('/perubahan_data/mentor/save')}}" method="post" enctype="multipart/form-data">
              @csrf
              <input type="hidden" name="id_mentor" value="{{$data_mentor->user_id}}">
              <input type="hidden" name="path_document" value="{{$data_mentor->path_document}}">
              <div class="form-group row">
                  ...
                  <div class="col-md-12" style="margin-bottom:1%;">
                    ...
                    <input id="detail_skill" class="col-md-12 form-control" type="text" name="detail_skill" maxlength="500" value="{{$data_mentor->detail_skill}}" placeholder="Misal : Saya ahli dalam beberapa bahasa, Software Engineer, Database Specialist, Project Management.(Maks. 200 Karakter)" required>
                  </div>
                  <div class="col-md-12" style="margin-bottom:3%;">
                    <label for="detail_skill">Biodata Singkat dan Latar Belakang</label>
                    <textarea id="biodata" class="col-md-12 form-control" name="biodata" maxlength="500" style="height:120%;" placeholder="Misal : Saya merupakan project manager pada salah satu perusahaan terkemuka di indonesia. Saya sudah banyak mengerjakan projek profesional baik swasta maupun instansi pemerintahan. Saya terbiasa menggunakan berbagai bahasa pemprograman mulai dari PHP, JavaScript, dan Java. Saya juga terbiasa dengan  HTML dan CSS serta beberapa framework pendukung seperti Laravel dan Codeigniter. Saya jga memahami metode pengembangan perangkat lunak dengan metode agile serta framework scrum." required>{{$data_mentor->biodata}}</textarea>
                  </div>
              </div><br>
              <div id="tambah_skill" class="form-group row">
                  ...
                  @foreach($detail_skill as $key => $value)
                    <div id="div_keahlian{{$key}}" class="form-group row col-md-12">
                      <!-- <div class="col-md-3">
                      </div> -->
                      <div class="col-md-3">
                        <label for="keahlian">Nama Keahlian</label>
                        <select id="keahlian{{$key}}" onfocus="this.size=5;" onblur="this.size=5;" onchange="this.size=5; this.blur();"
                        onfocusout="this.size=null;" class=" col-md-12 form-control" name="keahlian[]" onclick="pilih_skill({{$key}});" required>
                          @foreach($skill as $val)
                            @if($value->skill_id == $val->id)
                              <option value="{{$val->id}}" selected>{{$val->skill}}</option>
                            @else
                      ...
                      <div class="col-md-3">
                        ...
                        <input id="pengalaman_bidang{{$key}}" class="col-md-12 form-control" type="number" name="pengalaman_bidang[]" value="{{$value->lama_pengalaman}}" placeholder="Lama Pengalaman Di Bidang" required>
                      </div>
                      @if($key != 0)
                      <div class="col-md-3">
                        <button type="button" class="btn btn-sm btn-danger" onclick="hapus_keahlian({{$key}})"><i class="fas fa-times"></i></button>
                      </div>
                      @endif
                      <!-- <div class="col-md-12">
              ...
              <div class="form-group row">
                    ...
                    <label for="" class="col-md-12">Hari Anda Dapat Mengajar</label>
                      @foreach($hari as $key => $val)
                          <div class="row col-md-6">
                            <div class="col-md-3">
                              @if($val->nilai != 0)
                                <input id="hari_mengajar{{$val->id}}" type="checkbox" class="ml-3" name="hari_mengajar[]" value="{{$val->id}}" onclick="checkbox({{$val->id}})" checked> {{$val->hari}}
                              @else
                                <input id="hari_mengajar{{$val->id}}" type="checkbox" class="ml-3" name="hari_mengajar[]" value="{{$val->id}}" onclick="checkbox({{$val->id}})" > {{$val->hari}}
                              @endif
                            </div>
                            <div class="col-md-8">
                              @if($val->nilai != 0)
                                <input id="start_jam{{$val->id}}" name="start_jam[]" value="{{$val->start_jam}}" type="time">
                                <label>s/d</label>
                                <input id="end_jam{{$val->id}}" name="end_jam[]" value="{{$val->end_jam}}" type="time">
                              @else
                                <input id="start_jam{{$val->id}}" name="start_jam[]" type="time" disabled>
                                <label>s/d</label>
                                <input id="end_jam{{$val->id}}" name="end_jam[]" type="time" disabled>
                              @endif
                            </div>
                          </div>
                      @endforeach
                  ...
                  <div class="col-md-12 mt-5" style="margin-bottom:1%;">
                    ...
                    @foreach($tipe_pengajaran as $val)
                      @if($val->nilai != 0)
                          <input type="checkbox" class="ml-3" name="tipe_pengajaran[]" value="{{$val->id}}" checked> {{$val->tipe}}
                      @else
                          <input type="checkbox" class="ml-3" name="tipe_pengajaran[]" value="{{$val->id}}"> {{$val->tipe}}
                      @endif
                    @endforeach
                  </div>
                  <div class="col-md-12">
                    <label for="harga_jam">Harga /Jam</label>
                    <input id="harga_jam" class="col-md-12 form-control" type="text" name="harga_jam" value="Rp. {{number_format($data_mentor->harga_perjam,0,',','.')}}" placeholder="Harga Perjam" required>
                  </div>
                  <div class="col-md-12" style="margin-bottom:2%;">
                    <label for="pengalaman_ngajar">Pengalaman Mengajar (Dalam Tahun)</label>
                    <input id="pengalaman_ngajar" class="col-md-12 form-control" type="number" name="pengalaman_ngajar" value="{{$data_mentor->pengalaman_ngajar}}" placeholder="Pengalaman Mengajar (dalam tahun)." required>
                  </div>
              </div>
              <div class="form-group row">
                  ...
                  <div class="col-md-12">
                    ...
                    <input id="medsos_linkedin" class="col-md-6 form-control" type="text" name="medsos_linkedin" value="{{$data_mentor->medsos_linkedin}}" placeholder="Https://www.linkedin.com/in" required>
                  </div>
                  <div class="col-md-12">
                    <label for="medsos_github" class="col-md-12">Github</label>
                    <input id="medsos_github" class="col-md-6 form-control" type="text" name="medsos_github" value="{{$data_mentor->medsos_github}}" placeholder="@Github" required>
                  </div>
                  <div class="col-md-12" style="margin-bottom:2%;">
                    <label for="medsos_instagram" class="col-md-12">Instagram</label>
                    <input id="medsos_instagram" class="col-md-6 form-control" type="text" name="medsos_instagram" value="{{$data_mentor->instagram}}" placeholder="@Instagram" required>
                  </div>
              </div>
              <div class="form-group row">
              ...
              <div class="form-row" style="margin-left:20%;">
                  <div class="form-group mt-4 mb-5 col-md-8">
                    ...
                    <button type="submit" class="btn btn-primary" onclick="return confirm('Apakah Anda Yakin Dengan Data Anda?')"><i class="fa fa-save"></i> Ajukan Perubahan</button>
                  </div>
              </div>
            </form>
          </div>
         ...
         respon = $.parseJSON(response);
            if(respon.response == 'success')
            {
                ...
                var value = "<div id="+id_div_next+" class='form-group row col-md-12'>"
                                            ...
                                            +"<div class='col-md-3'>"
                                              +"<label for='keahlian'>Nama Keahlian</label>"
                                              +"<select id="+id_skill_next+" onfocus='this.size=5;' onblur='this.size=5;' onchange='this.size=5;"
                                              +"this.blur();' onfocusout='this.size=null;'class='col-md-12 form-control' onclick='pilih_skill("+jml+");' name='keahlian[]' required>"
                                                  +"<option value=''>-- Pilih Keahlian Anda --</option>";
                if(tot_data != 0){
                   for(a=0; a<tot_data; a++)
                   {
                     value = value +"<option value="+data[a].id+">"+data[a].skill+"</option>";
                   }
                    value = value +"</select>"
                                    +"</div>"
                                    +"<div class='col-md-3'>"
                                      +"<label for='pengalaman_bidang'>Lama Pengalaman (Dalam Tahun)</label>"
                                      +"<input id="+id_pengalaman_next+" class='col-md-12 form-control' type='number' name='pengalaman_bidang[]' placeholder='Lama Pengalaman Di Bidang' required>"
                                    +"</div>"
                                    +"<div class='col-md-3'>"
                                      +"<button type='button' class='btn btn-sm btn-danger' onclick='hapus_keahlian("+jml+")'><i class='fas fa-times'></i></button>"
                                    +"</div>"
                                    // +"<div class='col-md-12'>"
                                    //   +"<label class='col-md-12' for='deskripsi_bidang'>Deskripsi Singkat Dengan Keahlian Tersebut</label><br>"
                                    //   +"<textarea class='form-control col-md-12' id='deskripsi_bidang' name='deskripsi_bidang[]' placeholder='Misal: Saya terbiasa menggunakan bahasa tersebut untuk pembuatan program POS Main.'></textarea>"
...
function hapus_keahlian(val) {
  ...
  $('#div_pengalaman_bidang'+val).remove();
  $('#div_btn_times'+val).remove();
}
