...
class UserReservasiController extends Controller
{
  ...
  {
      DB::beginTransaction();
      $mentor_id = $request->id_mentor;
      $tgl_class = $request->tgl_class;
      $jam_class = $request->jam_class;
      $cek_kelas = DB::connection()->select("select id, tgl_class, jam_class, jumlah_jam, mentor_id,
                                                    DATE_ADD(jam_class, INTERVAL jumlah_jam HOUR) selesai_jam
                                             FROM tbl_reservasi
                                             WHERE mentor_id = $mentor_id
                                             AND tgl_class = '$tgl_class'
                                             AND status_id NOT IN (7,9,11,12)
                                             AND '$jam_class' BETWEEN jam_class
                                             AND DATE_ADD(jam_class, INTERVAL jumlah_jam HOUR)
                                            ");
      if(count($cek_kelas) != 0){
        DB::rollback();
        return Redirect::back()->with('failed', 'Pengajuan Reservasi Anda Failed, Tgl dan Jam Tidak Tersedia !!!');
      }else{
        ...
        $pengajuan_reservasi->tgl_class = $tgl_class;
        $pengajuan_reservasi->jam_class = $jam_class;
