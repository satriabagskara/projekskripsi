            ...
            @if(count($data_pengajuan) == 0)
              <div class="row">
                <div class="card col-sm-12 mb-3 text-center">
                  <h5 class="text-center text-danger"><i>Tidak Ada Data Pengajuan Perubahan Data Mentor.</i></h5>
                </div>
              </div>
            @else
                ...
                <div class="row">
                  <div class="card col-sm-1 mb-3 text-center">
                    @if($value->user->fhoto != null)
                      <a><img src="{{url('/Foto_profile/'.$value->user->path_fhoto,$value->user->fhoto)}}" class="img-circle" style="width:70%;margin-top:10%;"></a>
                    @else
                      <a><img src="{{url('/TemplateHome/dist/img/user.jpg')}}" class="img-circle" style="width:70%;margin-top:10%;"></a>
                    @endif
                  </div>
                  ...
                  <div class="card col-sm-2 text-center">
                    @if($value->status_id == 14)
                      <div class="row mb-1">
                        <a href="{{url('/pengajuan_perubahan/approve/'.$value->id)}}" type="button" class="btn btn-sm btn-success col-sm-6" title="Approve Pengajuan"><i class="fas fa-check"></i> Terima</a>
                        <a href="{{url('/pengajuan_perubahan/reject/'.$value->id)}}" type="button" class="btn btn-sm btn-danger col-sm-6" title="Reject Pengajuan"><i class="fas fa-times"></i> Tolak</a>
                    ...
                    @else
                      <br>
                    @endif
                    <a href="{{url('/manage/detail_perubahan_data/'.$value->id)}}" type="button" class="btn btn-sm btn-info"><i class="fas fa-eye"></i> Cek Detail</a>
...
<!-- modal for reject perubahan data mentor -->
<div class="modal fade" id="Modal_reject" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
    <form class="form-horizontal" action="{{url('/pengajuan_perubahan/reject')}}" method="POST">
      @csrf
    ...
    $.ajax({
         ...
         respon = $.parseJSON(response);
            if(respon.response == 'success')
            {
                 ...
                 $('#Title_reject').empty();
                 $('#Title_reject').append('<h3 class="modal-title text-primary" id="Title_reject">Penolakan Pengajuan '+nama+' Perubahan Data Mentor</h3>');
            }else{
